document.addEventListener('DOMContentLoaded', () => {
    ...
    if (checkWordsBtn) {
        checkWordsBtn.addEventListener('click', () => {
            ...
            const w4 = document.getElementById('word4').value.trim();
            const w5 = document.getElementById('word5').value.trim();

            // Remove all punctuation and convert to lowercase
            const sanitize = (str) => str.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()'"']/g, "").trim().toLowerCase();

            // Expected words also need to be sanitized if they contained punctuation, but "you've" becomes "youve"
            const expectedWordsSanitized = expectedWords.map(sanitize);

            if (
                sanitize(w1) === expectedWordsSanitized[0] &&
                sanitize(w2) === expectedWordsSanitized[1] &&
                sanitize(w3) === expectedWordsSanitized[2] &&
                sanitize(w4) === expectedWordsSanitized[3] &&
                sanitize(w5) === expectedWordsSanitized[4]
            ) {
                // Success
